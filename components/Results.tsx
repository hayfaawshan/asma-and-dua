  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center space-y-3 py-6">
        <MoonLoader />
        <p className="text-sm text-[var(--muted-foreground)] text-center">Finding the best matching Name...</p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-xl space-y-6">
      {safeCurrent.map((item, index) => (
        ...
        >
          ...
          <p className="text-sm text-[var(--muted-foreground)]">{item.meaning}</p>
          <p className="mt-2 text-sm">{item.reason}</p>
        </div>
      ))}

      {noMoreNames && (
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card-bg-subtle)] p-4 text-sm text-[var(--muted-foreground)]">
          ...
          {showPrevious && (
            <div className="space-y-4">
              {safePrevious.map((item, index) => (
                <div
                  key={`prev-${index}`}
                  className="rounded-lg border border-[var(--border)] bg-[var(--card-bg-subtle)] p-4"
                >
      ...
      <p className="text-sm text-[var(--muted-foreground)]">Call upon Allah with these Names in your duʿāʾ.</p>
    </div>
  );
}
